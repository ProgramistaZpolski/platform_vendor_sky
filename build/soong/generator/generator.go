...
func init() {
	android.RegisterModuleType("sky_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = skyExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = skyExpandVariables(ctx, dep_file)
	...
	cmd := skyExpandVariables(ctx, String(g.properties.Cmd))
