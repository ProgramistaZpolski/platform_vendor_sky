function __print_sky_functions_help() {
cat <<EOF
Additional SkyDroid functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- skygerrit:   A Git wrapper that fetches/pushes patch from/to SkyDroid Gerrit Review.
- skyrebase:   Rebase a Gerrit change and push it again.
- skyremote:   Add git remote for SkyDroid Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for SkyDroid Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the sky model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch sky_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/sky-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.sky.device | grep -q "$sky_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $sky_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function skyremote()
{
    ...
    local sky_USER=$(git config --get review.review.skydroid.org.username)
    if [ -z "$sky_USER" ]
    then
        git remote add sky ssh://review.skydroid.org:29418/$PFX$PROJECT
    else
        git remote add sky ssh://$sky_USER@review.skydroid.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'sky' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/SkyDroid/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.sky.device | grep -q "$sky_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        skyremote
        git push sky HEAD:refs/heads/'$1'
    fi
    '
}
...
function skygerrit() {
    ...
    local user=`git config --get review.review.skydroid.org.username`
...
function skyrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "SkyDroid Gerrit Rebase Usage: "
        echo "      skyrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.skydroid.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.sky.device | grep -q "$sky_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/sky/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $sky_FIXUP_COMMON_OUT ]; then
